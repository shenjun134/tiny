				...
				function() {
					...
					function begin() {
						if (tp1) {
							...
							ctx.shadowBlur = 20;
							ctx.shadowColor = color2;
							ctx.arc(cx + 50 * Math.cos(p * Math.PI / 180), cy
									+ 50 * Math.sin(p * Math.PI / 180), 10, 0,
									Math.PI * 2);
							ctx.closePath();
							ctx.fill();
							ctx.beginPath();
							ctx.arc(cx + 50
									* Math.cos((p + 180) * Math.PI / 180), cy
									+ 50 * Math.sin((p + 180) * Math.PI / 180),
									10, 0, Math.PI * 2);
							ctx.closePath();
							ctx.fill();
							ctx.beginPath();
							ctx.arc(cx + 50
									* Math.cos((p + 90) * Math.PI / 180), cy
									+ 50 * Math.sin((p + 90) * Math.PI / 180),
									10, 0, Math.PI * 2);
							ctx.closePath();
							ctx.fill();
							ctx.beginPath();
							ctx.arc(cx + 50
									* Math.cos((p + 270) * Math.PI / 180), cy
									+ 50 * Math.sin((p + 270) * Math.PI / 180),
									10, 0, Math.PI * 2);
							ctx.closePath();
							ctx.fill();
							ctx.shadowBlur = 0;
							...
							for (var i = 0; i < y.length; i++) {
								...
								ctx.lineTo((y[i].wx + y[i].x) / 2
										+ Math.random() * 20,
										(y[i].wy + y[i].y) / 2 + Math.random()
												* 20);
							...
							for (var i = cx - 60; i < cx + 60; i += 4) {
								for (var j = cy - 60; j < cy + 60; j += 4) {
									if (Math.sqrt(Math.pow(cx - i, 2)
											+ Math.pow(cy - j, 2)) <= 60) {
										ctx.globalAlpha = 1 - (Math.sqrt(Math
												.pow(cx - i, 2)
												+ Math.pow(cy - j, 2)) / 60);
										if (Math.random() < .2) {
					...
					/*
					 * c.mousedown(function(){ c.on('mousemove',function(e){ cx =
					 * e.pageX-c.offset().left; cy = e.pageY-c.offset().top;
					 * y.push({x:cx,y:cy,r:b.r,o:1}); });
					 * c.on('mouseup',function(){ c.off('mouseup');
					 * c.off('mousemove'); c.off('moseleave'); });
					 * c.on('mouseleave',function(){ c.off('mouseup');
					 * c.off('mousemove'); c.off('moseleave'); }); });
					 */
					$("#btn1").click(function() {
						tp1 = true;
